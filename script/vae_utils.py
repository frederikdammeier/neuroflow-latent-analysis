...
from mindeye2 import BrainNetwork, BrainNetwork_M1, PriorNetwork, BrainDiffusionPrior
...
def load_mindeye1(args):
    
    model = MindEyeModule()
    
    model.backbone = BrainNetwork_M1(in_dim=15724, out_dim=257*768, clip_size=768, use_projector=True)
    ...
    depth = 6
    dim_head = 64
    heads = clip_size//64 # heads * dim_head = clip_emb_dim
    timesteps = 100
...
def load_mindeye2(args):
    ...
    model.backbone = BrainNetwork(h=hidden_dim, in_dim=hidden_dim, seq_len=1, blurry_recon=True,
                            clip_size=clip_emb_dim, out_dim=clip_emb_dim*clip_seq_dim)
    count_params(model.ridge) ## 64,409,600
    count_params(model.backbone)  ## 1,903,020,028
    # count_params(model)
    
    total = 0
    trainable = 0
    for name, p in model.backbone.named_parameters():
        if name.startswith(("blin1", "bnorm", "bupsampler", "b_maps_projector")):
            total += p.numel()
            if p.requires_grad:
                trainable += p.numel()
    print(f"Blurry recon total params: {total / 1e6:.3f} M") ## 15.159 M
    print(f"Blurry recon trainable params: {trainable / 1e6:.3f} M")  ## 15.159 M
...
def mindeyev2_generate(model, x, args, return_all=False):
    ...
    if args.subject in [1,2,5,7]:
        ...
        if args.subject == 8:
            out = model.ridge(x, args.subject - 2)
        else:
            out = model.ridge(x, args.subject - 1)
    print(out.shape)
...
def mindeyev2_blurry(model, x):
    ...
    autoenc.to(device)
    count_params(autoenc)
    
    x = x.to(device)
    out = model.ridge(x, 0)
    print(out.shape)
    backbone, clip_voxels, blurry_recon = model.backbone(out)
    print(backbone.shape, clip_voxels.shape)
    print(blurry_recon[0].shape)
    
    blurry_image = (autoenc.decode(blurry_recon[0]/0.18215).sample/ 2 + 0.5).clamp(0,1)
    # all_blurry_recons = []
    # for i in range(len(blurry_image)):
    #     im = torch.Tensor(blurry_image[i])
    #     all_blurry_recons.append(im[None])
